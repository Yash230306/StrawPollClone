    ...
    <div className="space-y-4">
      {options.map((o, i) => (
        <div key={o.id} className="rounded-xl border border-slate-200 p-4 space-y-3 bg-slate-50/50">
          ...
          <Input value={o.imageUrl} onChange={e => update(o.id, 'imageUrl', e.target.value)} placeholder="Image URL (https://...)" className="h-10" />
          {o.imageUrl && (
            <img src={o.imageUrl} alt="" className="w-full h-28 object-cover rounded-lg border border-slate-200" onError={e => e.target.style.display='none'} />
          )}
        </div>
      ))}
          ...
          {pollType !== 'meeting' && (
            <Card className="p-6 border-slate-200">
              ...
              <div className="space-y-4">
                ...
                <SettingRow title="Hide results until vote" description="Results are only shown after a user votes." checked={hideResults} onChange={setHideResults} />
              </div>
            </Card>
          )}
