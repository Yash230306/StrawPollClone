...
function AuthShell({ children, side }) {
  return (
    <div className="min-h-screen flex">
      {/* Left panel – branding */}
      <div className="hidden lg:flex flex-col justify-between w-5/12 bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-700 p-12 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 relative z-10">
          <div className="w-10 h-10 rounded-xl bg-white/20 backdrop-blur flex items-center justify-center">
            <span className="w-5 h-5 rounded-full bg-white/90 block" />
          </div>
          <span className="text-2xl font-extrabold text-white tracking-tight">
            Straw<span className="text-purple-200">Poll</span>
          </span>
        </div>
        ...
        <div className="relative z-10">
          ...
          <p className="mt-4 text-purple-200 text-base leading-relaxed">
            {side === 'login'
              ? 'Log in and manage all your polls, see live results, and share with your audience.'
              : 'Join millions of users who use StrawPoll to make decisions together — fast and for free.'}
          </p>

          <div className="mt-10 space-y-4">
            {[
              { icon: BarChart3, text: 'Real-time poll results & charts' },
              { icon: Users, text: 'Share with anyone, no account needed' },
              { icon: Zap, text: 'Create a poll in under 30 seconds' },
              { icon: Shield, text: 'Bot & VPN protection built-in' },
            ].map(({ icon: Icon, text }) => (
              <div key={text} className="flex items-center gap-3 text-purple-100">
        ...
        {/* Social proof */}
        <div className="relative z-10 flex items-center gap-3 bg-white/10 backdrop-blur rounded-xl p-4">
          <div className="flex -space-x-2">
            {['SC', 'MJ', 'ER', 'BK'].map((init) => (
              <div key={init} className="w-8 h-8 rounded-full bg-gradient-to-br from-pink-400 to-violet-400 flex items-center justify-center text-white text-xs font-bold border-2 border-white/30">
                {init}
              </div>
            ))}
          </div>
          <div>
            <p className="text-white text-sm font-semibold">2.3M+ active users</p>
            <p className="text-purple-200 text-xs">290M+ votes cast</p>
          </div>
        </div>
      </div>
